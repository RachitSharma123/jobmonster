...
class AdzunaAdapter(ApplyAdapter):
    """Opens Adzuna redirect URL — browser follows to the real job page and fills it."""
    name = "adzuna"
    default_mode = ApplyMode.ASSISTED

    def can_handle(self, job: JobLead, page_text: str = "") -> bool:
        return detect_platform(job.apply_url) == "adzuna"
    ...
    def build_plan(self, job: JobLead, candidate: CandidateProfile, docs: Documents) -> ApplyPlan:
        ...
        return ApplyPlan(
            ...
            warnings=("Adzuna redirect — browser follows to real job page then fills form.",),
        )
